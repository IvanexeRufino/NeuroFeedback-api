...
create table treatment (id integer generated by default as identity, version bigint not null, name varchar(50) not null, user_id integer not null, description varchar(255) not null, primary key (id));
create table treatment_parameter (id integer generated by default as identity, version bigint not null, stimulus_id integer not null, max_value double not null, frecuency double not null, min_value double not null, primary key (id));
create table treatment_treatment_parameter (treatment_treatment_parameters_id integer not null, treatment_parameter_id integer);
create table user (id integer generated by default as identity, version bigint not null, doc_type varchar(5) not null, doc_number varchar(50) not null, first_name varchar(50) not null, role_id integer not null, last_name varchar(50) not null, date_of_birth timestamp not null, email varchar(255) not null, primary key (id));
create table user_history (id integer generated by default as identity, version bigint not null, user_id integer not null, effectiveness double not null, duration double not null, primary key (id));
...
alter table user_history add constraint UK_9ywgm5uty801t52mj674nel49 unique (duration);
...
alter table treatment_treatment_parameter add constraint FKg3ea75inpaotyos8wuro86lmy foreign key (treatment_parameter_id) references treatment_parameter;
alter table treatment_treatment_parameter add constraint FK2r4mckh1rthlos781r3u0aqoi foreign key (treatment_treatment_parameters_id) references treatment;
alter table user add constraint FKn82ha3ccdebhokx3a8fgdqeyy foreign key (role_id) references role;
alter table user_history add constraint FKaa6ilb6iqih95bntoeyysb2pc foreign key (user_id) references user;
create table permission (id integer generated by default as identity, version bigint not null, name varchar(50) not null, description varchar(255) not null, primary key (id));
create table role (id integer generated by default as identity, version bigint not null, name varchar(50) not null, description varchar(255) not null, primary key (id));
create table role_permission (role_permissions_id integer not null, permission_id integer);
create table stimulus (id integer generated by default as identity, version bigint not null, name varchar(50) not null, resource varchar(255) not null, primary key (id));
